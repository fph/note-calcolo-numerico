...
Quindi il metodo del punto fisso ha convergenza \emph{almeno lineare}. Perché ``almeno''? Perché può succedere che $\abs{\Phi'(\alpha)} = 0$. In questo caso, si ha quindi anche
\[
    \lim_{n\to\infty} \frac{e_{n+1}}{e_n} = 0.
\]
Questo comportamento implica una convergenza con una pendenza più veloce di quella di qualunque retta su un grafico in scala logaritmica: per ogni $r>0$ si ha da un certo punto in poi $\frac{e_{n+1}}{e_n} < r$, e quindi la successione degli errori va a zero più velocemente di una retta di pendenza $\log r$. Questo per qualunque $r$; per esempio, i punti potrebbero formare una parabola con la concavità verso il basso. Quando una successione soddisfa $\lim \frac{e_{n+1}}{e_n} = 0$, si dice che ha \emph{superlineare}. Questo tipo di convergenza si ha per la quarta successione in Figura~\ref{fig:iterativi}, che converge più velocemente degli altri metodi. Studieremo questo metodo nel dettaglio più avanti.
